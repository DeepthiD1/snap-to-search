...
async function main() {
  ...
  let saved = 0;
  const targetCount = Math.max(1, payload.size ?? 10);
  ...
  for (const item of results) {
    ...
    if (fs.existsSync(outPath)) {
      console.log(`- skip ${listingId} (already downloaded)`);
      continue;
    }
    ...
    try {
      ...
      saved += 1;
      console.log(`[${saved}/${targetCount}] saved ${listingId}${ext} (${Math.round(bytes / 1024)} KB)`);
    } catch (e: any) {
