...
async function fetchMlsDetail(listingId: string): Promise<ListingSummary | null> {
  ...
  try {
    ...
    // If MLS doesn’t provide a consumer URL, fall back to a Google search using the address
    if (!listingUrl && addressLine) {
      const q = [addressLine, city, state, zip].filter(Boolean).join(' ');
      listingUrl = `https://www.google.com/search?q=${encodeURIComponent(q)}`;
    }
