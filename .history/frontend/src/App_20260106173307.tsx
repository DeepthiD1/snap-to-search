...
const API_BASE_URL = (import.meta.env.VITE_API_BASE ?? 'http://localhost:4000').trim();
...
type PHashMatch = {
  ...
  distance: number;
  hash: string;
  previewImageUrl: string; // backend returns: /static/mls_images_10/<file>
};
...
function App() {
  ...
  const [pHashQuery, setPHashQuery] = useState<string | null>(null);
  const [pHashTop, setPHashTop] = useState<PHashMatch[]>([]);
  ...
  const handleSubmit = async () => {
    ...
    resetResults();
    setUiState({ loading: true, info: 'Preparing search…' });
    ...
    try {
      ...
      if (!loc) {
        ...
        try {
          ...
          setUiState({
            loading: false,
            error: 'Please allow location access so we can search nearby listings.',
          });
          return;
        }
      }
      ...
      if (!loc) {
        setUiState({ loading: false, error: 'Location unavailable.' });
        return;
      }

      setUiState({ loading: true, info: 'Comparing photo with nearby listings…' });
      ...
      if (!response.ok) throw new Error(payload.error ?? 'Failed to run search');

      updateResults(payload);

      // If backend returns 0 matches, show a helpful message instead of "nothing happens"
      if (!payload.matches?.length) {
        setUiState({ loading: false, info: 'No matches found. Try expanding search or using a clearer facade photo.' });
      }
    } catch (error) {
  ...
  const handlePHashTest = async () => {
    ...
    resetResults();
    setUiState({ loading: true, info: 'Finding closest matches (pHash)…' });
    ...
    try {
      ...
      if (!response.ok) throw new Error(payload.error ?? 'pHash test failed');

      setPHashQuery(payload.queryHash);
      setPHashTop(payload.top ?? []);
      setUiState({
        loading: false,
        info: payload.top?.length ? undefined : 'No pHash matches found.',
      });
    } catch (error) {
      setUiState({ loading: false, error: (error as Error).message });
    }
  ...
  const handleExpand = async () => {
    ...
    try {
      ...
      if (!response.ok) throw new Error(payload.error ?? 'Failed to expand search');

      updateResults(payload);

      if (!payload.matches?.length) {
        setUiState({ loading: false, info: 'Still no matches. Try a different photo.' });
      }
    } catch (error) {
      setUiState({ loading: false, error: (error as Error).message });
    }
  ...
  const updateResults = (payload: SearchResponsePayload & { nextActionToken?: string }) => {
    ...
    setUiState({
      loading: false,
      info: payload.status === 'expanded' ? 'Expanded nearby search applied' : undefined,
    });
  };
  ...
  return (
    <main className="app-shell">
      <header>
        ...
        <p>Upload a facade photo and we’ll compare it against nearby active listings.</p>
      </header>
      ...
      <section className="capture-panel">
        ...
        <p className="hint">
          We use your device location only to find nearby listings to compare against. It isn’t stored.
        </p>
        ...
        <div className="button-row">
          ...
          <button type="button" className="cta secondary" disabled={!readyToSubmit} onClick={handlePHashTest}>
      ...
      {pHashTop.length > 0 && (
        <section className="results-panel">
          <div className="results-header">
            <div>
              <h2>pHash top matches</h2>
              {pHashQuery && <p>Query hash: {pHashQuery}</p>}
            </div>
            <span className="status-pill">phash</span>
          </div>

          <div className="matches-grid">
            {pHashTop.map((m) => {
              const imgUrl = m.previewImageUrl.startsWith('http')
                ? m.previewImageUrl
                : `${API_BASE_URL}${m.previewImageUrl}`;

              return (
                <article key={m.listingId} className="match-card">
                  <img src={imgUrl} alt={m.listingId} />
                  <div className="match-body">
                    <h3>{m.listingId}</h3>
                    <p>
                      Distance: <strong>{m.distance}</strong> (lower = more similar)
                    </p>
      ...
      {matches.length > 0 && (
        <section className="results-panel">
          <div className="results-header">
            <div>
              <h2>Top matches</h2>
              {candidateCount !== null && <p>{candidateCount} nearby homes checked.</p>}
            </div>
